...
代码见 `main.go` ， 需要自行按需修改，具体怎么使用见下方使用教程，t 的值使用软件怎么获取这里不描述，自行探索，懂的都懂，感谢 issues 贡献方法老铁们，集思广益汇集力量，本内容会随时间发生改变，请自行分辨。
...
### 使用教程
1、 克隆本仓库地址或者下载文件到本地，安装好 go 1.19 环境
```shell
git clone https://github.com/joker8023/sheep-assistant.git
```
2、安装依赖
```go
go mod tidy
...
1. 本仓库发布的 `sheep-assistant` (下文均用本项目代替) 项目中涉及的任何脚本，仅用于测试和学习研究，禁止用于商业用途，不能保证其合法性，准确性，完整性和有效性，请根据情况自行判断。
